...
export default function Dashboard({ data, filter, customFrom, customTo, dailyStats }) {
  ...
  const s = card => ({ background: '#161618', borderRadius: 12, padding: '22px 24px', ...card })
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'nowrap' }}>
        {rates.map(r => (
          <div key={r.label} style={{ flex: 1, minWidth: 0, background: '#161618', borderRadius: 12, padding: '22px 24px' }}>
            <div style={{ fontSize: 11, color: '#666669', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>{r.label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
        {isWeekend
          ? ['Outreach', 'Followups', 'Pos. Followups'].map(l => (
            <div key={l} style={s({})}>
              <div style={{ fontSize: 11, color: '#666669', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 10 }}>{l}</div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#333336', lineHeight: 1, marginTop: 8 }}>Not today</div>
            </div>
          ))
          : [
