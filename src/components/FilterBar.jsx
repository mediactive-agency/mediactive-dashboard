...
export default function FilterBar({ active, onFilter, customFrom, customTo, onCustomFrom, onCustomTo, onCustomApply }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
      <div style={{ display: 'flex', gap: 2, background: 'var(--filter-bg)', border: '1px solid var(--border)', borderRadius: 9, padding: 3, transition: 'background 0.3s' }}>
        {FILTERS.map(f => (
          <button
            key={f.key}
      ...
      {active === 'custom' && (
        <div style={{ display: 'flex', background: 'var(--filter-bg)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{ padding: '8px 18px', background: 'var(--filter-active-bg)', color: 'var(--filter-active-text)', border: 'none', borderRadius: 7, cursor: 'pointer', fontWeight: 700, fontSize: 12 }}
          >Apply</button>
        </div>
      )}
    </div>
