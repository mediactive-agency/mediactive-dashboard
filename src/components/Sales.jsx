...
const ICO = {
  ...
  cross:    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>,
...
// Upcoming calls are loaded dynamically from Calendly via proxy
...
export default function Sales({ data, filter, customFrom, customTo, isMobile, isTablet }) {
  const { filtered, linkedinMap, upcomingCalls, stats } = useMemo(() => {
    if (!data) return { filtered: [], linkedinMap: {}, upcomingCalls: [], stats: null }
    ...
    // Upcoming calls z Calendly (proxy vrací čistá data s invitee jménem)
              ...
              <div style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                  ...
                  {c.confirmed ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, background: '#F5973318', color: '#F59733', borderRadius: 999, padding: '3px 8px', fontSize: 12, fontWeight: 700 }}>
                      {ICO.cross} Not confirmed yet
                    </div>
                  )}
                </div>
