...
// Track one PFU series into dailyPFUTotal/Done dicts
// replyDate: when positive reply happened
// slotStart: first column index of followup slots
// slotCount: number of slots (7)
...
function trackPFUSeries(r, replyDate, slotStart, slotCount, bookedDate, calDays, todayStr, dailyPFUTotal, dailyPFUDone) {
  ...
  for (let i = 0; i < slotCount; i++) {
    ...
    const sd = toDateStr(r[slotStart+i])
  ...
  for (const D of calDays) {
    ...
    if (endedAsOfD || filledAsOfD === slotCount) {
      if (D === todayStr) {
        for (let i = 0; i < slotCount; i++) {
...
export function computeTaskStats(data, { vslMode = false } = {}) {
  ...
  allRows.forEach(x => {
    ...
    // booked date depends on mode
    const bookedDate = vslMode ? toDateStr(r[40]) : toDateStr(r[27])
    const hasPositiveReply = !!(r[14] && toDateStr(r[14]))

    if (!hasPositiveReply) {
      // Regular followup (next biz day after initiation)
  ...
  calDays.forEach(D => {
    ...
    const d2 = (dailyFUDone[D]||0)+(dailyPFUDone[D]||0)
    if (t > 0) dailyFollowupTotal[D] = t
    if (d2 > 0) dailyFollowupDone[D] = d2
  })
